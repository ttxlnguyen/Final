async function createAChannel(channelData) {
  try {
    const token =
    ...
    if (response.ok) {
      ...
      document.getElementById('channel-container').innerText = `Error: ${response.statusText}`;
    }
  } catch (error) {
    console.error('Fetch Error:', error);
    document.getElementById('channel-container').innerText = `Fetch Error: ${error.message}`;
  }
}
